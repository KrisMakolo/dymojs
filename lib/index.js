...
class Dymo {
	...
	print(printerName, labelXml, copies=1, printQuality='Text', labelSetXml='') {
		const params = `<LabelWriterPrintParams><Copies>${copies}</Copies><PrintQuality>${printQuality}</PrintQuality></LabelWriterPrintParams>`;

		let label = `printerName=${encodeURIComponent(printerName)}&printParamsXml=${encodeURIComponent(params)}&labelXml=${encodeURIComponent(labelXml)}&labelSetXml=${encodeURIComponent(labelSetXml)}`;
