...
class Dymo {
	...
	print(printerName, labelXml, copies = 1, printQuality = 'Text', labelSetXml = '') {
		const printingParams = `<LabelWriterPrintParams><Copies>${copies}</Copies><PrintQuality>${printQuality}</PrintQuality></LabelWriterPrintParams>`

		let label = `printerName=${encodeURIComponent(printerName)}&printParamsXml=${encodeURIComponent(printingParams)}&labelXml=${encodeURIComponent(labelXml)}&labelSetXml=${encodeURIComponent(labelSetXml)}`

		console.log(label)
		console.log('------------')
		console.log(decodeURI(label))
